...
coef_int_all <- coef_int_all %>%
  mutate(
    ...
    across(c(beta, std.error, conf.low, conf.high, partial_R2, f2,p_bh), ~ round(.x, 3)),
    CI = sprintf("(%.3f, %.3f)", conf.low, conf.high)) %>%
  select(interaction, year, beta, CI, p.value, p_bh, p, partial_R2, f2, effect_size) %>%
  arrange(interaction, year)

coef_sig <- coef_int_all %>%
  filter(!is.na(p.value) & p_bh < 0.05) %>%
  arrange(interaction, year)
